...
void list_pyosi_c(struct SUJA_LIST *sijak) {
    struct SUJA_LIST *p;
    int i;
...
int main() {
    ...
    printf("\n<삽입후 원형 연결 리스트 조회(리스트 끝에 노드 추가)>\n");

    list_pyosi_c(sijakptr -> daeum);

    printf("\n --------------------------------------\n");

    printf("\n<삽입후 원형 연결 리스트 조회(n번 노드 추가)> \n");
    printf("몇번째와 숫자 입력[x 빈칸 y].....? => "); scanf("%d %d", &n, &suja);

    node_sabip(n, suja, sijakptr);

    printf("\n<삽입후 원형 연결 리스트 조회(리스트 n번째에 노드 추가)>\n");

    list_pyosi_c(sijakptr -> daeum);

    printf("\n -------------------------------------\n");
    printf("<삭제후 원형 연결 리스트 조회(n번 노드 삭제)> \n");
